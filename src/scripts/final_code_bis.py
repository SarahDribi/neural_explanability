...
from src.explanation_logic.nap_extraction.nap_utils import (
    get_num_kept,
    get_num_total_neurons,
    

)
from src.explanation_logic.verification_utils.unchanged_activations import ( get_unchanged_activations
,export_unchanged_activations)
...
from src.explanation_logic.dataset.utils import get_label_input,get_label_inputs
from src.explanation_logic.dataset.other_dataset import get_correctly_classified_inputs_breast_cancer
from src.explanation_logic.dataset.other_dataset import get_label_inputs_breast_cancer
...
# default mode 
default_heuristics: List[str] = ["simple","preactivation_impact","descending","random1","random2","local_entropy","optimized"]


default_model: str = "model.onnx"
# also dedault dataset is mnnist 
...
class ExplanationInput:
    ...
    #invariant_activation_states:List[List[int]] 
    # It highlights the neurons whose activation state doesn't vary in 
    # the epsilon ball around input 
    # aka region constraint => imposes fixed activation states of
    # those neurons
    # 
...
def get_explanation_input(
    ...
    try: 
        ...
        if data_set_name=="breast_cancer":
            num_classes=2
        region_constrained_neuron_act=get_unchanged_activations(model_path,input_tensor,epsilon,nap,predicted_label,num_classes=num_classes)
    except Exception as e:
        print(f"Failed to see the unchanged neuron status {e}")
    ...
    region_constrained_activations=region_constrained_neuron_act
)
...
def add_image_to_results_folder(base_path: str, dataset: str, image_idx: int) -> str:
    return get_full_filename(base_path, dataset, image_idx, "input.pt")
...
def export_input_tensor(image: Any, path: str):
    """
    Sauvegarde un tenseur PyTorch au format .pt
    """
    if isinstance(image, torch.Tensor):
        torch.save(image, path)
    else:
        raise TypeError(f"Type non supporté pour l'export: {type(image)}")
...
,exp_input:ExplanationInput):
    rows: List[Dict[str, Any]] = [extract_info(r) for r in results]
    unchanged_neuron_activations=exp_input.region_constrained_activations
    try:
    ...
    try:
        ...
        export_input_tensor(image, img_path)
        print(f"Input écrit: {img_path}")
    except Exception as e:
    ...
    try:
        unchanged_activations_path=get_full_filename(base_path,dataset,image_idx,"constrained_activations_by_region.txt")
        export_unchanged_activations(unchanged_neuron_activations,unchanged_activations_path)
    except Exception as e:
        print(f"Exporting unchanged activations failed")
...
def explain_input(
    ...
    # I will add a timestamp  to distinguish results 
    log_results(results, dataset=data_set_name, image_idx=input_id, image=input_data.data, base_path=results_folder_name,exp_input=input_data)
    # I should also save the unchanged neurons
     
    return results



def explain_many_images_test_set(num_samples,data_set_name: str = "mnist",onnx_path: str = "models/mnist-10x2.onnx",results_folder_name="results_mnist"):
    
    for label in [1,3,7,9,6,8,2]:
...
def explain_breast_cancer_images(num_to_explain: int = 1,data_set_name: str = "breast_cancer",onnx_path: str = "artifacts/wbc_mlp.onnx",results_folder_name="results_breast_cancer"):
...
def arg_parser():
    import argparse
    # I also should add another base path 
...
if __name__ == "__main__":
    ...
    if args.dataset == "mnist":
        explain_many_images_test_set(num_samples= args.num_images,data_set_name=args.dataset,onnx_path=args.model,results_folder_name=folder_name)
    elif args.dataset == "breast_cancer":
        explain_breast_cancer_images(num_to_explain= args.num_images,data_set_name=args.dataset,onnx_path=args.model)
    else:       
